...
struct Learner {
  Oracle &orc; const Mealy &ref; int K; EQMode eq; int mExtra; CEMode cem = CEMode::RS;
  std::vector<std::pair<long,long>> rsLog;   // (|CE|, binary-search queries)
  ...
  Word rivestSchapire(const Word &ce) {
    ...
    int lo = 0, hi = k; long q0 = st.rsBin; long iters = 0;
    while (hi - lo > 1) { int mid = lo + (hi-lo)/2; iters++; if (beta(mid) != hOut(mid)) lo = mid; else hi = mid; }
    rsLog.push_back({(long)k, iters}); (void)q0;
  ...
  std::optional<Word> eqRW() { std::uniform_int_distribution<int> ua(0, K-1); std::uniform_real_distribution<double> ur(0,1);
    st.suite = rwTests;
    for (int t = 0; t < rwTests; t++) { Word w; w.push_back(ua(rng));
      while (ur(rng) > rwStop && w.size() < 80) w.push_back(ua(rng));
  ...
  Stats run() { fillAll(); while (closeOnce()) {} buildHyp();
    for (int g = 0; g < 20000; g++) { st.rounds++; auto ce = equivalence(); if (!ce) break;
      ...
      if (cem == CEMode::RS) E.push_back(rivestSchapire(*ce));
      else { std::set<Word> have(E.begin(), E.end());
             for (size_t i = 0; i < ce->size(); i++) { Word sf(ce->begin()+i, ce->end()); if (have.insert(sf).second) E.push_back(sf); } }
...
static void hdr() { std::printf("%-22s %-5s %-10s %-9s | %8s %8s %9s | %5s %4s %4s %6s | %8s %7s %8s | %s\n",
    "machine","n","CE mode","EQ oracle","mq","resets","symbols","|S|","|E|","rnd","maxCE","fillQ","rsBin","eqQ","ok"); }
static void one(const char *tag, const Mealy &ref, CEMode c, const char *cn, EQMode e, int mx, const char *en,
                bool cacheOn, int rt = 20000) {
  Oracle orc(ref); orc.useCache = cacheOn; Learner L(orc, ref, e, mx, rt); L.cem = c; auto s = L.run();
  std::printf("%-22s %-5d %-10s %-9s | %8ld %8ld %9ld | %5d %4d %4ld %6ld | %8ld %7ld %8ld | %s\n",
      tag, ref.n, cn, en, s.mq, s.resets, s.symbols, s.states, s.esize, s.rounds, s.ceMax,
      s.fillQ, s.rsBin, s.eqQ, s.correct ? "CORRECT" : "WRONG");
  if (c == CEMode::RS && !L.rsLog.empty()) {
    std::printf("%22s   RS per-CE: ", "");
    for (auto &pr : L.rsLog) { long need = 0; while ((1L << need) < pr.first) need++;
      std::printf("|CE|=%ld->%ldq(ceil log2=%ld) ", pr.first, pr.second, need); }
    std::printf("\n"); }
}

int main() {
  Base b = tcpBase();
  std::printf("=== Rivest-Schapire vs Maler-Pnueli, perfect EQ oracle (isolates CE handling) ===\n"); hdr();
  for (int L : {0, 2, 5, 10, 20}) {
    Mealy p = tcpTimesCounter(b, L);
    char tag[64]; std::snprintf(tag, sizeof tag, "tcp x counter L=%d", L);
    one(tag, p, CEMode::RS, "RS 1 sufx", EQMode::Perfect, 0, "perfect", true);
    one(tag, p, CEMode::MP, "MP all sfx", EQMode::Perfect, 0, "perfect", true);
    std::printf("\n");
  }
  std::printf("=== forced-long counterexamples: mod-n counter, |Sigma|=6 ===\n"); hdr();
  for (int n : {10, 25, 40, 80}) {
    Mealy m; m.n = n; m.k = 6; m.nout = 2; m.iname = {"a","b","c","d","e","f"};
    m.d.assign(n, std::vector<int>(6)); m.o.assign(n, std::vector<int>(6));
    for (int s = 0; s < n; s++) for (int a = 0; a < 6; a++) {
      if (a == 0) { m.d[s][a] = (s+1)%n; m.o[s][a] = (s == n-1); } else { m.d[s][a] = s; m.o[s][a] = 0; } }
    char tag[64]; std::snprintf(tag, sizeof tag, "mod-%d counter", n);
    one(tag, m, CEMode::RS, "RS 1 sufx", EQMode::Perfect, 0, "perfect", true);
    one(tag, m, CEMode::MP, "MP all sfx", EQMode::Perfect, 0, "perfect", true);
    std::printf("\n");
  }
  return 0;
}
