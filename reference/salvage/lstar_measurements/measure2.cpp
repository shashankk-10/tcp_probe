...
static void one(const char *tag, const Mealy &ref, EQMode e, int mx, const char *en, bool cacheOn, int rt = 20000) {
  Oracle orc(ref); orc.useCache = cacheOn; Learner L(orc, ref, e, mx, rt); auto s = L.run();
...
int main() {
  ...
  for (int L : {0, 1, 2, 3, 5, 10}) {
    Mealy p = tcpTimesCounter(b, L);
    std::printf("  limit L=%-2d  reachable states=%-4d distinguishable=%-4d\n", L, p.n, minimalSize(p));
  }
  std::printf("\n=== TCP base x challenge counter, EQ oracles compared (cache ON) ===\n"); hdr();
  for (int L : {0, 1, 2, 3, 5, 10}) {
    Mealy p = tcpTimesCounter(b, L);
    char tag[64]; std::snprintf(tag, sizeof tag, "tcp x counter L=%d", L);
    one(tag, p, EQMode::Perfect, 0, "perfect", true);
    one(tag, p, EQMode::W,  1, "W m=n+1", true);
    one(tag, p, EQMode::Wp, 1, "Wp m=n+1", true);
    one(tag, p, EQMode::W,  2, "W m=n+2", true);
    one(tag, p, EQMode::Wp, 2, "Wp m=n+2", true);
    one(tag, p, EQMode::RandomWalk, 0, "rw 2k p=.06", true, 2000);
    one(tag, p, EQMode::RandomWalk, 0, "rw 20k p=.06", true, 20000);
    std::printf("\n");
  }
  std::printf("=== same, cache OFF (raw resets to the SUL) ===\n"); hdr();
  for (int L : {0, 3, 10}) {
    Mealy p = tcpTimesCounter(b, L);
    char tag[64]; std::snprintf(tag, sizeof tag, "tcp x counter L=%d", L);
    one(tag, p, EQMode::Perfect, 0, "perfect", false);
    one(tag, p, EQMode::Wp, 1, "Wp m=n+1", false);
    one(tag, p, EQMode::RandomWalk, 0, "rw 20k p=.06", false, 20000);
    std::printf("\n");
  }
  return 0;
}
